...
#define SEM_MUTEX_SUFFIX "mutex_"
...
static char sem_mutex_name[SEM_NAME_MAX];
...
static int parse_positive(char *arg);
...
int mvar_main(int argc, char *argv[]) {
    ...
    int num_writers = parse_positive(argv[0]);
    int num_readers = parse_positive(argv[1]);
...
static int writer_process(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        sys_sem_wait(sem_mutex_name);

        mvar_value = letter;

        sys_sem_post(sem_mutex_name);
